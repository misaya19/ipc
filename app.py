# mysql查询特定时间段内的数据：https://blog.csdn.net/u013991521/article/details/80872899
# Flask—RESTful官方文档:http://www.pythondoc.com/Flask-RESTful/reqparse.html
...
class HelloWorld(Resource):
    def get(self):
        ...
        a = Microwave.query.filter(Microwave.station_id == '54511', Microwave.datatype == '11',
                                   Microwave.datetime >= args['time_down'], Microwave.datetime <= args['time_up']).all()
...
class GetTemperature(Resource):
    ...
    def get_data(self):
        parser = reqparse.RequestParser()
        # parser.add_argument('page', type=int, help='Page error')  # 分页
        parser.add_argument('time_down', type=str, help='Time_down error')
        parser.add_argument('time_up', type=str, help='Time_up error')
        args = parser.parse_args()
        self.all_data['Time frame'] = args['time_down'] + '---' + args['time_up']
        # per_pages = 0
        # one_data = Microwave.query.filter(Microwave.station_id == '54511', Microwave.datatype == '11').paginate(page=args['page'], per_page=720)  # 分页
        one_data = Microwave.query.filter(Microwave.station_id == '54511',
                                          Microwave.datatype == '11',
        ...
        # xxx = Microwave.query.filter(Microwave.station_id == '54511', Microwave.datatype == '11').first()
        # print(xxx.datetime.day)
        # self.data = one_data.items    # 分页
        self.data = one_data
...
api.add_resource(GetTemperature, '/data/')  # 分页：
api.add_resource(GetTemperature, '/data/')  # 取范围时间：http://127.0.0.1:5000/data/?time_down=2018-07-01%2000:08:00.000000&time_up=2018-07-01%2000:58:00.000000
